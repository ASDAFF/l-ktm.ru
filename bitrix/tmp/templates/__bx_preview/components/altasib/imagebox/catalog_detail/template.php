...
{
?>
	<a rel = "<?=$zoom?>" href="<?=$big_pict?>"   title="<?=$name?>" style = 'width: <?=$width?>px; height: <?=$height?>px; margin-top: -<?=$height/2?>px; margin-left: -<?=$width/2?>px'>
	<img src = '<?=$small_pict?>'  />
	</a>
<?
}
...
<?if (count($arResult["MORE_PHOTO"]) > 1 || $arParams["SHOW_DETAIL_PICTURE"] == 1):?>
        <?foreach ($arResult["MORE_PHOTO"] as $key => $value):?>
					<?if($value['ID']){?>
                <a href="<?=$arResult["MORE_PHOTO"][$key]["SRC"]?>" class="alx_thumbs_item"><span style="background-image:url(<?=$arResult["MORE_PHOTO"][$key]["thumb_src"]["src"]?>)"><img src="<?=$templateFolder?>/images/spacer.gif" alt="" /></span></a>
					<?}?>
        <?endforeach;?>
<?endif;?>
