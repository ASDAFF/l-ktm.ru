...
<?foreach($arResult["ITEMS"] as $arItem):?>
	<div class="NEW_div<?if($bOdd=!$bOdd) {?> odd<?}?>">
		...
		<div class="NEWS_body">
			...
			<?if($arParams["DISPLAY_PICTURE"]!="N" && is_array($arItem["PREVIEW_PICTURE"])):?>
				...
				<a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><img class="preview_picture" src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" width="<?=$arItem["PREVIEW_PICTURE"]["WIDTH"]?>" height="<?=$arItem["PREVIEW_PICTURE"]["HEIGHT"]?>" alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" style="float:left;margin-right:5px;" /></a>
				<?else:?>
